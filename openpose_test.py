...
class Openpose(object):
    ...
    def train(self, resume=False):
        ...
        for ((imgs, pafs, heatmaps, ignore_mask), xyz) in zip(tqdm(iter(self.train_loader)), range(0, 1)):
            # 定向攻击设置
            # pafs_targeted = pafs
            # print(pafs.shape)
        ...
        for ((imgs, pafs, heatmaps, ignore_mask), xyz) in zip(tqdm(iter(self.train_loader)), range(0, 5000)):
            ...
            per = per.transpose(2, 0, 1)
            per = np.expand_dims(per, axis=0)
            pers = Variable(torch.from_numpy(per).float())
            
            imgs, pafs, heatmaps, ignore_mask, pers = imgs.to(self.device), pafs.to(self.device), heatmaps.to(
                self.device), ignore_mask.to(self.device), pers.to(self.device)
            ...
            for i in range(0, epochs):
                ...
                '''PGD攻击算法'''
                pers.data = 0.05 * torch.sign(imgs.grad.data)
                imgs.data = imgs.data - 0.05 * torch.sign(imgs.grad.data)
                # print(heatmaps_loss_1)
                
                per = pers.cuda().data.cpu().numpy()[0]  # 四维变三维
                mean = [0.485, 0.456, 0.406]
                std = [0.229, 0.224, 0.225]
                per = per.transpose(1, 2, 0)
                per = (per * std) + mean
                per = per * 255.0
                per = np.clip(per, 0, 255).astype(np.uint8)
                
                '''Opencv生成图像'''
                per = per[..., ::-1]  # RGB to BGR
                per = cv2.resize(per, (image_width, image_height), interpolation=cv2.INTER_CUBIC)
                img_attacked_path = 'output/adv_{}.jpg'.format(2)
                cv2.imwrite(img_attacked_path, per)  # 保存全部攻击的图片
                
                '''deepfool_2攻击算法'''
                # image_a = imgs.grad.data.cpu()
                # total_loss_1 = total_loss.data.cpu()
                # print(total_loss_1)
                # if total_loss_1 > 100:
                    # break
                # r = image_a * total_loss_1 / pow(np.linalg.norm(image_a), 2)  #DEEPFOOL
                # r_cuda = r.data.cuda()
                # imgs.data = imgs.data + r_cuda
                '''
                对抗样本
                '''
